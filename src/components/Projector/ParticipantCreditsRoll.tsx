...
export const ParticipantCreditsRoll: React.FC<ParticipantCreditsRollProps> = ({
  ...
  if (!shouldAnimate) {
    return (
      <div className="w-full flex flex-wrap items-center justify-center gap-3 py-2 animate-in fade-in zoom-in-95">
        {participants.map((p, idx) => (
          <div
            key={p.id + '-' + idx}
            className={`flex items-center space-x-3 px-4 py-2.5 rounded-xl border-2 border-[#1E1E1E] text-xs font-bold transition-all ${
              isDark
          ...
          >
            <span
              className="w-8 h-8 rounded-lg border-2 border-[#1E1E1E] flex items-center justify-center text-sm shrink-0"
              style={{ backgroundColor: p.avatarBg }}
            >
              {p.avatarEmoji || '🦊'}
            </span>
            <div className="min-w-0 text-left">
              <span className={`font-black text-sm block truncate max-w-[220px] leading-tight ${
  ...
  const gridColsClass = {
    ...
    4: 'grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6',
  }[effectiveColumns];
